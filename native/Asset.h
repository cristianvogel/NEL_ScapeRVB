...
public:
    ...
    static inline elem::js::Value toJsValue()
    {
        elem::js::Value value;
        // todo
        return value;
...
private:
  ...
  // 2. Specialization for juce::File
    template<> 
    inline const juce::File& Asset::get<juce::File>(Props property) const {
        switch (property) {
    ...
    template<> 
    inline const std::string& Asset::get<std::string>(Props property) const {
        switch (property) {
    ...
    template<> 
    inline const std::vector<float>& Asset::get<std::vector<float>>(Props property) const {
        switch (property) {
