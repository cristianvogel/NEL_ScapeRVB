    ...
    namespace js
    {
        ...
        public:
             ...
             Value(Function const& v) : var(v)
            {
             ...
             Value(std::shared_ptr<Asset> const& asset) : var(asset)
            {
            }

            Value(const std::map<SlotName, Asset>& assetsMap) {
                 Object obj;
                 for (const auto& [key, value] : assetsMap) {
                     // I assume here that you have << operator overloaded for Asset & SlotName
                     // And they convert Asset and SlotName respectively to std::string
                     std::stringstream key_ss;
                     key_ss << slotname_to_string(key) ;
                     // Your Asset class should have a method called `to_value` to convert
                     // Asset object to Value
                     obj[key_ss.str()] = value.wrap_asset();
                 }
                 var = obj;
             }
            ...
            bool isAsset() const { return std::holds_alternative<std::shared_ptr<Asset>>(var); }
            ...
            std::shared_ptr<Asset> const& getAsset() const { return std::get<std::shared_ptr<Asset>>(var); }
            ...
            std::shared_ptr<Asset>& getAsset() { return std::get<std::shared_ptr<Asset>>(var); }
            ...
            String toString() const
            {
                ...
                if (isArray())
                {
                    ...
                    for (size_t i = 0; i < std::min(static_cast<size_t>(3), a.size()); ++i)
                        ss << std::to_string(static_cast<double>(a[i])) << ", ";
                ...
                if (isFloat32Array())
                {
                    ...
                    for (size_t i = 0; i < std::min(static_cast<size_t>(3), fa.size()); ++i)
