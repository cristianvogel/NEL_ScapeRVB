...
choc::network::HTTPContent
EffectsPluginProcessor::ViewClientInstance::getHTTPContent(
    std::string_view path) {
  // not using HTML
  return {};
}
void EffectsPluginProcessor::ViewClientInstance::upgradedToWebSocket(
    std::string_view path) {}
void EffectsPluginProcessor::ViewClientInstance::handleWebSocketMessage(
  ...
  if (parsed.isObject()) {
    ...
    for (auto &[key, value] : socketMessage) {
       ...
       */
      if (key == "selectFiles" &&
          value.isNumber()) // the number value is the high pass cutoff
                            // frequency choice
      {
        ...
        for (const auto fileValue : files) {
          auto filePath =
              static_cast<juce::String>(elem::js::String(fileValue));
          ...
          if (file.existsAsFile()) {
            ...
            processor.activeImpulseResponses[currentUserSlot] =
                file; // uncertain about this.... why are there two File ref
                      // vectors?
            // now do the loading and processing
            processor.loadAudioFromFileIntoVFS(filePath, currentUserSlot);
            // and generate the reduced buffer data that will be used for
            // plotting peaks in the View
      ...
      if (key == "requestState") {
        ...
        std::size_t currentStateHash =
            std::hash<std::string>{}(serializedState);
      ...
      if (value.isNumber() && processor.parameterMap.count(key) > 0) {
        ...
        if (key == "srvbBypass" || key == "scapeBypass" ||
            key == "scapeReverse" || key == "scapeMode") {
...

{
  ...
  const auto parameters =
      manifest.getWithDefault("parameters", elem::js::Array());
...
#else
  auto assetsDir =
      getAssetsDirectory().getChildFile("assets/impulse-responses");
#endif
...
void EffectsPluginProcessor::resgisterDefaultImpulseResponsesToVFS(
    std::vector<juce::File> &impulseResponses) {
  for (auto &file : impulseResponses) {
    ...
    auto name = choc::text::toUpperCase(
        file.getFileNameWithoutExtension()
            .toStdString()); // "Ambience_0.wav" -> "AMBIENCE_0"
    ...
    nel::normaliseImpulseResponse(
        buffer, 0.8414); // -1.5 db
                         // add the gain ramped impulse response to the virtual
                         // file system
    // ▮▮▮elem▮▮▮runtime▮▮▮
    runtime->updateSharedResourceMap(name, buffer.getReadPointer(0),
                                     numSamples);
    ...
    runtime->updateSharedResourceMap(REVERSE_BUFFER_PREFIX + name,
                                     buffer.getReadPointer(1),
                                     numSamples * 0.75);
  }
  inspectVFS();
}
std::vector<juce::File> EffectsPluginProcessor::sortOrderForDefaultIRs(
    std::vector<juce::File> &impulseResponses) {
  ...
  std::sort(impulseResponses.begin(), impulseResponses.end(),
            [&order](const juce::File &a, const juce::File &b) {
              ...
              auto aPos = std::find_if(order.begin(), order.end(),
                                       [&aName](const std::string &keyword) {
                                         return aName.find(keyword) !=
                                                std::string::npos;
                                       });
              auto bPos = std::find_if(order.begin(), order.end(),
                                       [&bName](const std::string &keyword) {
                                         return bName.find(keyword) !=
                                                std::string::npos;
                                       });
              return aPos < bPos;
            });
...
void EffectsPluginProcessor::updateStateWithPeaksData() {
  ...
  userData.insert_or_assign(PERSISTED_USER_PEAKS,
                            elem::js::Value(reducedSampleDataForPlotting));
  dispatchStateChange();
}
...
void EffectsPluginProcessor::updateStateWithFilename(const juce::File &file) {
  ...
  state.insert_or_assign(PERSISTED_USER_FILENAMES,
                         elem::js::Value(userIRFilenames));
}
void EffectsPluginProcessor::requestUserFileSelection(
    std::promise<elem::js::Object> &promise) {
...
void EffectsPluginProcessor::loadAudioFromFileIntoVFS(juce::File file,
                                                      int slotIndex) {
  auto buffer = juce::AudioBuffer<float>();
  auto reader = formatManager.createReaderFor(file);
  ...
  if (reader == nullptr) {
    dispatchError("File error:",
                  "Could not read " + file.getFileName().toStdString());
    return;
  }
  ...
  for (int channel = 0; channel < numChannels; ++channel) {
    // source files are one channel, but we cunningly setup the second channel
    // for a derived 'shaped' version
    auto name =
        "USER" + std::to_string(slotIndex) + "_" + std::to_string(channel);
...
void EffectsPluginProcessor::inspectVFS() {
  ...
  const auto expr =
      serialize(jsFunctions::vfsKeysScript, choc::value::Value(vfsString), "%");
  sendJavascriptToUI(expr);
  jsContext.evaluateExpression(expr);
}
std::vector<float> EffectsPluginProcessor::reduceAudioBuffer(
  ...
  juce::AudioData::Pointer<
      juce::AudioData::Float32, juce::AudioData::LittleEndian,
...
#if ELEM_DEV_LOCALHOST
  uint16_t preferredPortNum =
      13755; // dev with fixed port. 0 for random port when building
#else
...
void EffectsPluginProcessor::createParameters(
    const std::vector<elem::js::Value> &parameters) {
  for (const auto &parameter : parameters) {
    if (!parameter.isObject())
      continue;

    auto paramId =
        parameter.getWithDefault("paramId", elem::js::String("unknown"));
    ...
    auto defaultValue =
        parameter.getWithDefault("defaultValue", elem::js::Number(0));
    auto step = parameter.getWithDefault("step", elem::js::Number(0));
    auto isBoolean =
        parameter.getWithDefault("isBoolean", elem::js::Boolean(false));

    if (isBoolean) {
      auto *p = new juce::AudioParameterBool(
          juce::ParameterID(paramId, 1), name, static_cast<bool>(defaultValue));
      ...
      parameterReadouts.emplace_back(
          ParameterReadout{static_cast<float>(defaultValue), false});
...
juce::AudioProcessorEditor *EffectsPluginProcessor::createEditor() {
  ...
  editor->handleUnlockEvent = [this](choc::value::Value &v) {
    const bool hasSerial = v.hasObjectMember("serial") &&
                           v["serial"].isString() &&
                           v["serial"].getString().length() > 0;
    ...
    sendJavascriptToUI("globalThis.__onUnlock__('" +
                       unlock::errorStatuses(licenseStatus) + "')");
    // also send back the current host info
    juce::PluginHostType hostType;
    std::string hostDescription =
        static_cast<const char *>(hostType.getHostDescription());
...
void EffectsPluginProcessor::prepareToPlay(double sampleRate,
  ...
  if (sampleRate != lastKnownSampleRate ||
      samplesPerBlock != lastKnownBlockSize) {
...
void EffectsPluginProcessor::handleAsyncUpdate() {
  ...
  if (shouldInitialize.exchange(false)) {
    runtime = std::make_unique<elem::Runtime<float>>(lastKnownSampleRate,
                                                     lastKnownBlockSize);

    runtime->registerNodeType(
        "convolver",
    ...
    // Update the front end with names derived from the VFS path for first
    // channel of each default IR
    auto files = {activeImpulseResponses[0], activeImpulseResponses[2],
                  activeImpulseResponses[4], activeImpulseResponses[6]};
  ...
  for (size_t i = 0; i < parameterReadouts.size(); ++i) {
    ...
    if (pr.dirty) {
      if (const auto *pf =
              dynamic_cast<juce::AudioParameterFloat *>(params[i])) {
        auto paramId = pf->paramID.toStdString();
        state.insert_or_assign(paramId,
                               static_cast<elem::js::Number>(pr.value));
      }
    }
  }
...
(function() {
  ...
  return true;
        ...
        if (auto *editor = static_cast<WebViewEditor *>(getActiveEditor())) {
          ...
          auto expr =
              juce::String(kDispatchScript)
                  .replace("%", elem::js::serialize(choc::json::toString(v)))
                  .toStdString();
 ...
 */
void EffectsPluginProcessor::dispatchError(std::string const &name,
 ...
 */
void EffectsPluginProcessor::dispatchNativeLog(std::string const &name,
                                               std::string const &message) {
  const auto expr = juce::String(jsFunctions::errorScript)
                        .replace("@", elem::js::serialize(name))
                        .replace("%", elem::js::serialize(message))
...
EffectsPluginProcessor::loadDspEntryFileContents() const {
 ...
 */
std::string
EffectsPluginProcessor::serialize(const std::string &function,
                                  const elem::js::Object &data,
...
std::string
EffectsPluginProcessor::serialize(const std::string &function,
                                  const choc::value::Value &data,
                                  const juce::String &replacementChar) {
  return juce::String(function)
      .replace(replacementChar, choc::json::toString(data))
      .toStdString();
}
...
void EffectsPluginProcessor::getStateInformation(juce::MemoryBlock &destData) {
  ...
  dataToPersist.insert_or_assign(PERSISTED_HOST_PARAMETERS,
                                 elem::js::Value(state));
  dataToPersist.insert_or_assign(
      PERSISTED_USER_PEAKS, elem::js::Value(userData[PERSISTED_USER_PEAKS]));
    ...
    for (auto &[key, value] : persistedData) {
      if (key == PERSISTED_HOST_PARAMETERS) {
        for (auto &[paramId, setting] : value.getObject()) {
          ...
          dispatchNativeLog("Restoring:", "Host parameter " + paramId + " to " +
                                              elem::js::serialize(setting));
        }
